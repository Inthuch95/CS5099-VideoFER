...
def extract_frames_from_video():
    ...
    if 'video_frames2' not in os.listdir('../'):
        os.mkdir('../video_frames2/')
    #go through video folder
    for emotion in emotions:
        # create label folder
        newdir = '../video_frames2/' + emotion
        if emotion not in os.listdir('../video_frames2/'):
            os.makedirs(newdir)
        for f in os.listdir(video_path+emotion):
            if '.mov' in f:
                filename = f.replace('.mov', '')
                video_file = video_path + emotion + '/' + f
                os.makedirs('../video_frames2/' + emotion + '/' + filename +  '/')
                filename = '../video_frames2/' + emotion + '/' + filename +  '/' + filename
...
def crop_face_from_frames():
    path = '../video_frames2/'
    dataset_path = '../dataset2/All/'
    count = 0 
    face_detector = dlib.get_frontal_face_detector()
    
    if 'dataset2' not in os.listdir('../'):
        os.mkdir('../dataset2/')
        if 'All' not in os.listdir('../dataset2/'):
            os.mkdir('../dataset2/All/')
    for emotion in emotions:
        print(emotion)
